      ...
      <div className="relative z-10 max-w-2xl mx-auto text-center">
        ...
        <div className="flex items-center bg-white border-2 border-[#1e2d4a]/10 rounded-full px-5 py-3 shadow-sm mb-6 max-w-lg mx-auto">
          <input
            ...
            readOnly
            className="flex-1 outline-none text-sm text-gray-600 placeholder-gray-400 bg-transparent cursor-pointer"
          />
          <button onClick={() => navigate('/login')} className="ml-3 bg-[#f5c518] rounded-full w-9 h-9 flex items-center justify-center hover:opacity-90 transition-opacity flex-shrink-0">
        ...
        {/* Category pills — navigate to login */}
          ...
          {categories.map((cat, i) => (
            <button
              ...
              className={`rounded-full px-5 py-2.5 font-bold text-sm transition-opacity hover:opacity-80 ${
                i % 2 === 0 ? 'bg-[#1e2d4a] text-white' : 'bg-[#f5c518] text-[#1e2d4a]'
              }`}
            >
              {cat}
            </button>
